/*
	...
	Translate to ChineseSimplified:
...
function private_dialog()
{
	...
	{		
		...
		if (this.opAddText)
		{
			...
			this.ui.newTextInput_label.text = "新文本:";			
			this.ui.prefixRB_label.text = "添加文本为:";
			this.ui.prefixRB_label.enabled = true;	
			this.ui.prefixRB.text = "前  缀            ";
			this.ui.prefixRB.enabled = true;
			this.ui.suffixRB.text = "后  缀";
		...
		else if (this.opReplace)
		{
			...
			this.ui.newTextInput_label.text = "替换:   ";
		...
		else
		{
			...
			this.ui.findInput.enabled = false;	
			this.ui.newTextInput_label.text = "新文本:";				
			this.ui.prefixRB_label.text = "排序:";
			this.ui.prefixRB_label.enabled = true;
			this.ui.prefixRB.text = "文本 + 序列号";			
			this.ui.prefixRB.enabled = true;
			this.ui.suffixRB.text = "序列号 + 文本";			
			this.ui.suffixRB.enabled = true;			
			...
			else
			{
				...
				if (this.upperCase)
				{
					this.ui.alphabetModel1RB.text = "A, B ... Z, ZA, ZB ... ( 用于较短序列 )";
					this.ui.alphabetModel2RB.text = "AA, AB ... AZ, BA, BB ... ( 最大 676 )";
	...
	this.buildPreview = function()
	{
		...
		if (exceedMaxLimit)
		{
			this.ui.previewInput.text = "超过了这个类型的极限!";
			this.ui.renameButton.enabled = false;			
		}
		else if (nameIsBlank)
		{
			this.ui.previewInput.text = "新名字不能为空!";
			this.ui.renameButton.enabled = false;	
		}
	...
	this.applyNewNames = function(celInfo, renameCount)
	{
		...
		{
			this.ui.previewInput.text = "无法重命名(名字已存在)";
			this.ui.renameButton.enabled = false;
			return renameCount;
		}
		...
		for (var idx = 0; idx < celInfo.selected.length; idx++)
		{
			...
			if (!renameSuccess2)
			{
				this.ui.previewInput.text = "重命名失败(请先连选多个绘图帧)";
				return -1;	
			}
		}
		...
		if (this.keepNewName)
		{
			...
			for (var i = 0; i < celInfo.excluded.length; i++)
			{
				if (celInfo.newName.indexOf(celInfo.excluded[i]) !== -1)
				{
					...
					if (!renameSuccess)
					{
						this.ui.previewInput.text = "重命名被排除的绘图帧失败";
						return -1;
					}
				}
			}
		...
		for (var idx2 = 0; idx2 < celInfo.selected.length; idx2++)
		{
			...
			if (!renameSuccess3)
			{
				this.ui.previewInput.text = "重命名失败";
				return -1;			
			}
			else
				renameCount++;
		...
		this.ui.previewInput.text = "总共重命名了 " + renameCount + " 个绘图帧";	
		return renameCount;
	};
}
...
{	
	...
	this.createUI = function()
	{
		...
		this.dialog.setWindowTitle("批量重命名绘图帧v" + scriptVer);	
		...
		this.dialog.opBox = new QGroupBox("操作类型");
		this.dialog.opBox.setStyleSheet("font: 12pt");
		...
		this.dialog.opRenameRB = new QRadioButton("序列重命名");
		this.dialog.opLayout.addWidget(this.dialog.opRenameRB, 0, 0);

		this.dialog.opAddTextRB = new QRadioButton("添加新文本");		
		this.dialog.opLayout.addWidget(this.dialog.opAddTextRB, 0, 0);

		this.dialog.opReplaceRB = new QRadioButton("替换/删除文本");	
		this.dialog.opLayout.addWidget(this.dialog.opReplaceRB, 0, 0);

		this.dialog.textLayout.setSpacing(15);	
		this.dialog.textBox = new QGroupBox("文本");
		this.dialog.textBox.setStyleSheet("font: 12pt");
		...
		this.dialog.findInput_label = new QLabel("查找:");	
		...
		this.dialog.newTextInput_label = new QLabel("新文本:");
		...
		this.dialog.prefixRB_label = new QLabel("排序:");		
		this.dialog.textRLayout.addWidget(this.dialog.prefixRB_label, 0, 0);	
		
		this.dialog.prefixRB = new QRadioButton("文本 + 序列号");		
		this.dialog.textRLayout.addWidget(this.dialog.prefixRB, 0, 0);

		this.dialog.suffixRB = new QRadioButton("序列号 + 文本");	
		this.dialog.textRLayout.addWidget(this.dialog.suffixRB, 0, 0);		

		this.dialog.seqBox = new QGroupBox("序列号");
		this.dialog.seqBox.setStyleSheet("font: 12pt");
		...
		this.dialog.addLeadZeroCB = new QCheckBox("开头用 0 顶位");
		this.dialog.numSubLayout.addWidget(this.dialog.addLeadZeroCB, 1, 0);		

		this.dialog.leadZeroDigitCombo_label = new QLabel("总共几位数字:");
		...
		this.dialog.reverseCB = new QCheckBox("倒序");	
		this.dialog.numLayout.addWidget(this.dialog.reverseCB, 0, 0);			

		this.dialog.alphaBox = new QGroupBox("字母选项");
		...
		this.dialog.upperCaseCB_label = new QLabel("模式:");
		this.dialog.alphaTopLayout.addWidget(this.dialog.upperCaseCB_label, 0, 0);	
		
		this.dialog.upperCaseCB = new QCheckBox("全部大写字母");
		this.dialog.alphaTopLayout.addWidget(this.dialog.upperCaseCB, 0, 0);

		this.dialog.alphabetModel1RB = new QRadioButton("a, b ... z, za, zb ...   ( 用于较短序列 )");
		this.dialog.alphaLayout.addWidget(this.dialog.alphabetModel1RB, 1, 0);

		this.dialog.alphabetModel2RB = new QRadioButton("Aa, Ab ... Az, Ba, Bb ...  ( 最大 676 )");	
		this.dialog.alphaLayout.addWidget(this.dialog.alphabetModel2RB, 1, 0);

		this.dialog.alphabetModel3RB = new QRadioButton("aa, ab ... az, ba, bb ...   ( 最大 676 )");		
		...
		this.dialog.startLetterCombo_label = new QLabel("起始字母:");
		...
		this.dialog.nsBox = new QGroupBox("遇到重名时的起名规则");
		this.dialog.nsBox.setStyleSheet("font: 12pt");
		...
		this.dialog.namespaceBox_label = new QLabel("当所选绘图帧之外重名时(在所选之外已经存在你想要起的新名字时)……");		
		this.dialog.nsLayout.addWidget(this.dialog.namespaceBox_label, 0, 0);
		
		this.dialog.nsLayout.addLayout(this.dialog.nsSubLayout);
		
		this.dialog.keepNewNameRB = new QRadioButton("所选之内起成新名字，所选之外在结尾加上数字");		
		this.dialog.nsSubLayout.addWidget(this.dialog.keepNewNameRB, 1, 0);

		this.dialog.suffixNewNameRB = new QRadioButton("所选之内在结尾加上数字");	
		...
		this.dialog.renameButton = new QPushButton("重命名");
		this.dialog.renameButton.setStyleSheet("font: 12pt");
		this.dialog.bottomLayout.addWidget(this.dialog.renameButton, 0, 0);
		
		this.dialog.closeButton = new QPushButton("关闭");
		this.dialog.closeButton.setStyleSheet("font: 12pt");
